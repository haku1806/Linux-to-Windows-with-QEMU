...
link2_status=$(curl -Is https://hakuit.com/WS2012R2.ISO | grep HTTP | cut -f2 -d" ")
...
if [ $link1_status = "200" ] ; then 
	...
	sudo wget -P /mediabots https://hakuit.com/WS2012R2.ISO
else
...
/tmp/qemu-system-x86_64 -net nic -net user,hostfwd=tcp::3389-:3389 -m 120G -localtime -enable-kvm -cpu host,+nx -M pc -smp 2 -vga std -usbdevice tablet -k en-us -cdrom /mnt/WS2012R2.ISO -hda /dev/sda -boot once=d -vnc :5
#!/bin/bash
#
#Vars
mounted=0
...
echo "helper called" 
###for i in $(ps aux | grep -i "qemu" | head -2 | tr -s '[:space:]' | cut -f2 -d" ") ; do echo "killing process id : "$i ; kill -5 $i ; done
#sleep 30
echo "un-mounting"
umount -l /mnt
sleep 10
